...
( nc -z -w 3 "$CONNECTOR_HOST" "$CONNECTOR_PORT" >/dev/null 2>&1 ) & pid=$!
...
# if [[ $( nc -w 5 -z "$CONNECTOR_HOST" "$CONNECTOR_PORT" </dev/null 2>&1; echo $? ) ]]; then
# else
# fi
...
echo
# This does introduce a python requirement. Is there a json parser available in newer macos that does not have python? 
...
echo "[step] Removing newlines from JSON"
retrievalResponse=${retrievalResponse//$'\n'/}
echo
echo "SIGNATURE RETRIEVAL REQUEST RESPONSE AFTER REMOVING NEWLINES:"
echo "$retrievalResponse"
echo 
...
# Strip carriage returns inserted by Windows
crt="${crt//$'\r'/}"
